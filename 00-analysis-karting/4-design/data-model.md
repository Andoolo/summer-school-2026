...
>   эта модель (вместе с `rental_price`, `geometry`, `meeting_point`, `free_rental_kits`)
...
>
> **Переход состояний при `deleteAccount` (R-009).** При удалении аккаунта:
> - **Активные брони** (`status = active`) переводятся в `cancelled` (системная отмена):
>   проставляется `cancelled_at`, **освобождаются места-карты и прокатные комплекты** в слоте —
>   как при обычной ранней отмене (`free_seats += seats_count`, `free_rental_kits += rental_count`).
...
| name | string | Название (напр. «Короткая», «Длинная») |
| description | string? (nullable) | Описательный текст конфигурации/заезда для карточки слота (SCR-003). Опциональный; может отсутствовать или быть `null` |
| type | enum (`novice`/`experienced`) | Тип: новичковая / опытная |
| capacity_cap | int | Потолок мест (новичковая ≤8, опытная ≤14) |
| duration_min | int | Длительность, мин (≈15–20 + инструктаж) |
| geometry | polyline | Геометрия трассы для выделенной линии на карте: массив координат `[lat,lng]` либо encoded polyline. Используется для карты Яндекс на SCR-003/SCR-006 |
...
| marshal_id | FK → Marshal | Назначенный маршал (может быть `null` — см. валидации ниже) |
| start_at | datetime (UTC) | Дата и время старта в UTC; **хранится в UTC**, **источник истины — сервер**. Клиент отображает в **локальной зоне устройства**; границы бизнес-правил (правило 2 часов, окно выдачи) — в **зоне центра**, право/тип отмены вычисляет сервер (R-021) |
| total_seats | int | Всего мест-картов (≤ capacity_cap конфигурации) |
| free_seats | int | Свободно мест-картов (расчётное/денормализованное) |
| free_rental_kits | int | Свободно прокатных комплектов экипировки (шлем + подшлемник). **Карт всегда центра** — это лимит прокатной экипировки, а не картов |
| price | money (RUB) | Цена за место-карт (валюта — рубли) |
...
> **`marshal_id` может быть `null`**: расписание может быть опубликовано до назначения маршала.
> В карточке/списке слота (SCR-002/SCR-003) при `marshal_id = null` поле маршала не показывается
> (или выводится «Маршал будет позже»); запись при этом доступна, если `free_seats > 0`.
...
| seats_count | int | Число мест-картов в записи (1–3: себя + гости). Только агрегат, без сущности BookingSeat (R-013) |
| rental_count | int | Сколько из мест — с прокатной экипировкой (0..seats_count). Агрегат, без BookingSeat (R-013) |
| status | enum (`active`/`cancelled`/`late_cancel`/`club_cancelled`) | Статус записи. `club_cancelled` — **«Отменён центром»**: возникает, когда слот отменён центром (`Slot.status = cancelled`); клиент видит бронь как отменённую не по своей инициативе (R-008) |
...
> Вариант экипировки выводится из `rental_count` (прокатные комплекты) и
...
> Карта трассы: `TrackConfig.geometry` (полилиния) рисует выделенную линию,
> `Slot.meeting_point*` задаёт пин и текст места сбора (см.
> [SCR-003](../3-design-brief/SCR-003-slot-card.md),
> [SCR-006](../3-design-brief/SCR-006-booking-details.md) и
> [foundations §8](../3-design-brief/00-foundations.md#8-карта-трассы-статичный-превью-яндекскарты).
> Источник тайлов и ключ Яндекс.Карт (Static API / Maps JS API) — параметр конфигурации, в модель
> не входит.
...
момент отмены (`slot.start_at` в UTC — источник истины); граница правила 2 ч считается в **зоне
центра**, «ровно 2 часа» трактуется как **ранняя** (`≥ 2 ч`, см.
[foundations §6](../3-design-brief/00-foundations.md#6-tone-of-voice-и-общая-микрокопия), R-021).
Отдельно: при отмене **слота центром** (`Slot.status → cancelled`) связанные брони переходят в
`club_cancelled` — **«Отменён центром»** (отмена не по инициативе клиента, R-008); места-карты/
прокатные комплекты в слоте при этом не релевантны (слот снят).
...
| `active` | Отмена, до старта `≥ 2 ч` | `cancelled` | Места-карты и прокатные комплекты **возвращаются** в слот | UC-2, FR-17 |
| `active` | Отмена, до старта `< 2 ч` | `late_cancel` | Места-карты и прокат **НЕ освобождаются**, штрафов нет | UC-2 A1, FR-18 |
...
> недоступна ([SCR-006 §состояния](../3-design-brief/SCR-006-booking-details.md), UC-2 E1). Статус
...
на UI: при `cancelled` запись недоступна ([SCR-003 §состояния](../3-design-brief/SCR-003-slot-card.md)).

```mermaid
flowchart LR
    Start((●)) --> scheduled
    scheduled -->|отменён центром/по погоде| cancelled
    cancelled --> End((◉))
```
...
| `cancelled` | Пометка «Заезд отменён центром» | Недоступна / скрыта |

## Ключевые инварианты (целостность данных)

- `Slot.free_seats = Slot.total_seats − Σ(active+late_cancel bookings.seats_count)` — место-карт
  при поздней отмене НЕ освобождается.
- `Slot.total_seats ≤ TrackConfig.capacity_cap` (новичковая ≤8, опытная ≤14).
- `Slot.free_rental_kits = исходный прокатный фонд слота − Σ(active+late_cancel bookings.rental_count)`
  — прокатный комплект при поздней отмене тоже НЕ освобождается (экипировка/карт простаивают,
  см. домен); общий прокатный фонд — 14 комплектов (по числу картов-мест).
- Только **ранняя** отмена возвращает места-карты и прокатные комплекты в слот (`cancelled`);
  `late_cancel` удерживает и место-карт, и прокатный комплект (FR-17/FR-18).
- Запись/отмена выполняются атомарно: овербукинг и двойная бронь исключены при параллельных
  операциях (NFR-8, NFR-9).
- **Карт всегда центра**: `seats_count` = число занятых картов-мест; выбор экипировки
  (`rental_count`) не влияет на `free_seats`, только на `free_rental_kits`.
