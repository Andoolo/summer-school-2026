...
<p align="center">
  <img src="screenshots/apex-waitlist-join.png" alt="Лист ожидания: мест нет, можно встать в очередь" width="23%"/>
  <img src="screenshots/apex-waitlist-waiting.png" alt="Лист ожидания: 2-й в очереди" width="23%"/>
  <img src="screenshots/apex-waitlist-offer.png" alt="Лист ожидания: место освободилось" width="23%"/>
  <img src="screenshots/apex-my-queues.png" alt="Профиль: мои очереди" width="23%"/>
</p>
<p align="center">
  <sub>Лист ожидания: мест нет · «2-й в очереди» · место освободилось, запись открыта · «Мои очереди» в профиле. Снято на локальном стенде с данными из сида, 390×844 @3x.</sub>
</p>
...
- **Лист ожидания.** Если мест нет, в заезд можно встать в очередь на 1–3 места. Когда место
  ...
  на экран заезда. В каталоге заполненный заезд подписан «есть лист ожидания», а тот, где
  человек уже стоит, — «2-й в очереди» или «Вам предложено место». Если бот не настроен,
  листа ожидания нет, и каталог о нём не пишет.
- **«Мои очереди» в профиле** — все очереди человека: место в очереди или срок предложения,
  переход на заезд и выход из очереди в один тап.
...
| **Вход в очередь блокирует заезд разделяемо (`FOR SHARE`), а человека — отдельной блокировкой** | Бронь и отмена меняют число мест и ждут, пока идёт проверка «мест нет», поэтому очередь не разойдётся с бронями. Входы разных людей друг друга не ждут. Блокировка на человека нужна для лимита в 5 очередей: без неё два одновременных входа в разные заезды вставали оба (гонка найдена тестом). |
...
**Как проверено:** модульные и интеграционные тесты бэкенда на PostgreSQL и 102 теста клиента,
