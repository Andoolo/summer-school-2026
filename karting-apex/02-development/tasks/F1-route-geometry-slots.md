...
Клиенту нужна геометрия трассы (ломаная координат), чтобы нарисовать линию заезды на карте
...
- `getSlot` (`01-analysis/api/slots/api.yaml`): «Полные данные слота для карточки (SCR-003):
  трасса с geometry…».
- SCR-003 (карточка слота) — линия трассы на карте; вход для F2.
...
> Union `Geometry` по контракту — oneOf: массив координат **или** строка (encoded polyline). По
> ревью (M2) обработаны обе формы: `isJSONString` различает jsonb по первому значащему байту →
> строка `FromGeometry1`, массив `FromGeometry0`. Строка в БД больше не роняет `/slots` в 500.
...
curl -s localhost:8090/slots -H "Authorization: Bearer $TOKEN"
# → route: { ..., "geometry": [[55.75,37.61],[55.752,37.615],[55.751,37.62],[55.748,37.619],[55.747,37.613],[55.75,37.61]], ... }

# detail (getSlot, SCR-003):
curl -s localhost:8090/slots/55555555-5555-5555-5555-555555555555 -H "Authorization: Bearer $TOKEN"
# → route.geometry = [[55.75,37.61],[55.752,37.615],[55.751,37.62],[55.748,37.619],[55.747,37.613],[55.75,37.61]]
```
...
geometry       = [[55.75, 37.61], [55.752, 37.615], [55.751, 37.62], [55.748, 37.619], [55.747, 37.613], [55.75, 37.61]]
```

✅ И `listSlots`, и `getSlot` возвращают `route.geometry`; координаты совпадают с сидом трассы
«Городское кольцо». Контракт (required geometry) выполнен; F2 разблокирована.
