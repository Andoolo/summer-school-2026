...
curl -s -X POST localhost:8090/auth/request-code -d '{"phone":"+79990000001"}'
→ {"code":"7529","resend_after_seconds":60,"ttl_seconds":300}   ✅ код есть

# PROD (APP_ENV=production) — код НЕ возвращается:
docker run --rm -d --network apex_default -e APP_ENV=production -e DATABASE_URL=... -p 8090:8090 backend-api
