...
- `/auth/refresh` зарегистрирован **вручную** (`RouterOptions.AuthRefresh`), т.к. сгенерированный
  транспорт auth ещё не содержит этого трассы (спека ушла вперёд кода). Регенерация OpenAPI —
...
> VerifyCode/Refresh/Logout с ротацией; ручной трасса /auth/refresh. push — вне скоупа по решению.
...
- `backend/internal/http/router.go`, `backend/cmd/api/main.go` — трасса `/auth/refresh`.
