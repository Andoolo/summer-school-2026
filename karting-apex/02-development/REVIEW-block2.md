...
тем, что сид кладёт только массивы (трассаы read-only), но контракт строку разрешает.
...
- **L4 · F2 гео edge-cases** (антимеридиан ±180°, полюса cos(lat)≤0) не обрабатываются. Для картинг-региона
...
| L4 | Гео edge-cases (антимеридиан/полюса) — нерелевантны картинг-региону, оставлено. | — |
