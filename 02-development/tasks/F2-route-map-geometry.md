# F2 · Фича: рисовать маршрут на карте по реальной `route.geometry` (клиент)
...
На карточке слота (SCR-003) карта должна показывать **линию реального маршрута**. Данные для этого
теперь отдаёт бэкенд — `route.geometry` (F1). Клиент же рисовал **захардкоженную** декоративную
линию, не связанную с координатами слота: разные маршруты выглядели одинаково.
...
- Добавлена проекция координат в точки канваса `projectRoute(...)`:
  ...
  - **единый масштаб** по обеим осям (форма маршрута не искажается) + центрирование;
  - ось Y инвертируется — север сверху.
- Если точек ≥ 2 — рисуется реальная ломаная (`Path` из спроецированных точек, `Stroke` c
  `StrokeJoin.Round`), пин — в первой точке маршрута.
...
```
Ожидаемо: на карточке слота линия маршрута повторяет реальные координаты `route.geometry`
(для сид-маршрута «Острова и каналы» — зигзаг по трём точкам `59.978,30.262 → 59.981,30.271 →
59.976,30.285`), пин — в стартовой точке. У слота без geometry — прежняя декоративная линия.
