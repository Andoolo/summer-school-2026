-- +goose Up
-- Расширение pgcrypto здесь не нужно и намеренно не создаётся.
--
-- Единственное, что из него использовалось, — gen_random_uuid(), а эта функция
-- входит в ядро PostgreSQL начиная с версии 13. Все окружения проекта работают
-- на 16 и выше, так что расширение было мёртвым наследием более старых версий.
--
-- Строка не просто лишняя, а вредная: на управляемом Postgres (Neon) CREATE
-- EXTENSION подвешивает соединение, и миграции не проходили вовсе. Побочно она
-- же вызывала гонку в тестах — расширение создаётся на всю базу, а тестовые
-- пакеты работают в своих схемах параллельно и конфликтовали друг с другом.
