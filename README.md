...
| `apex-db` | PostgreSQL | внутренняя сеть Render |
...
> У неё на бесплатном тарифе ограниченный срок жизни (30 дней с момента создания
> плюс 14 дней grace-периода) и лимит — одна бесплатная база на аккаунт. Из-за этого
> база была перенесена на другого провайдера, а описание в `render.yaml` сохранено
> как исходная конфигурация.
